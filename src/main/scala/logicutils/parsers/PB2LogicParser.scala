...
package logicutils.parsers

import com.typesafe.scalalogging.LazyLogging
import org.parboiled2._
import logicutils.logic.{Constant, Literal, LogicalExpression, Variable, Clause}
...
final class PB2LogicParser(val input: ParserInput) extends Parser {
  ...
  def Clause = rule {
    Atom ~ " :- " ~ BodyLiterals ~ optional(".") ~ EOI ~> ((x, y) =>
      logicutils.logic.Clause(head = x, body = y.elems.map(_.asInstanceOf[Literal])))
  }
